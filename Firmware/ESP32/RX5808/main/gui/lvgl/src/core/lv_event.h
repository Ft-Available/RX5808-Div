...
typedef enum {
    ...
    /** Input device events*/
    LV_EVENT_PRESSED=0x01,             /**< The object has been pressed*/
    LV_EVENT_PRESSING=0x02,            /**< The object is being pressed (called continuously while pressing)*/
    LV_EVENT_PRESS_LOST=0x03,          /**< The object is still being pressed but slid cursor/finger off of the object */
    LV_EVENT_SHORT_CLICKED=0x04,       /**< The object was pressed for a short period of time, then released it. Not called if scrolled.*/
    LV_EVENT_LONG_PRESSED=0x05,        /**< Object has been pressed for at least `long_press_time`.  Not called if scrolled.*/
    LV_EVENT_LONG_PRESSED_REPEAT=0x06, /**< Called after `long_press_time` in every `long_press_repeat_time` ms.  Not called if scrolled.*/
    LV_EVENT_CLICKED=0x07,             /**< Called on release if not scrolled (regardless to long press)*/
    LV_EVENT_RELEASED=0x08,            /**< Called in every cases when the object has been released*/
    LV_EVENT_SCROLL_BEGIN = 0x09,        /**< Scrolling begins*/
    LV_EVENT_SCROLL_END = 0x0A,          /**< Scrolling ends*/
    LV_EVENT_SCROLL=0x0B,              /**< Scrolling*/
    LV_EVENT_GESTURE = 0x0C,             /**< A gesture is detected. Get the gesture with `lv_indev_get_gesture_dir(lv_indev_get_act());` */
    LV_EVENT_KEY= 0x0D,                 /**< A key is sent to the object. Get the key with `lv_indev_get_key(lv_indev_get_act());`*/
    LV_EVENT_FOCUSED = 0x0E,             /**< The object is focused*/
    LV_EVENT_DEFOCUSED = 0x0F,           /**< The object is defocused*/
    LV_EVENT_LEAVE = 0x10,               /**< The object is defocused but still selected*/
    LV_EVENT_HIT_TEST = 0x11,            /**< Perform advanced hit-testing*/

    /** Drawing events*/
    LV_EVENT_COVER_CHECK= 0x12,        /**< Check if the object fully covers an area. The event parameter is `lv_cover_check_info_t *`.*/
    LV_EVENT_REFR_EXT_DRAW_SIZE = 0x13, /**< Get the required extra draw area around the object (e.g. for shadow). The event parameter is `lv_coord_t *` to store the size.*/
