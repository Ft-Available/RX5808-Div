...
static void page_scan_calib_timer_event(lv_timer_t* tmr)
{
    ...
    if (tmr == scan_calib_timer)
    {
        if (repeat_count == 47)
        {
            ...
            if (rssi_adc_min0 + rssi_adc_min1 + 200 < rssi_adc_max0 + rssi_adc_max1)
            {
                lv_label_set_text_fmt(calib_result_info, "Calibration success!\nThe result has\nbeen saved");
            ...
            {
                lv_label_set_text_fmt(calib_result_info, "Calibration failed!\nPlease try again!");
...
void page_scan_calib_create()
{
    ...
    lv_obj_set_style_border_width(open_vtx_cont, 2, LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_border_width(calib_start_cont, 2, LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_border_width(calib_result_cont, 2, LV_STATE_DEFAULT);
